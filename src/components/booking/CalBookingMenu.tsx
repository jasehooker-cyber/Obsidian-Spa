...
import { reportMetaBooking } from "@/lib/analytics/meta-pixel";
  ...
  useEffect(() => {
    ...
    (async () => {
      for (const namespace of CAL_NAMESPACES) {
        ...
        // Booking completes inside Cal's iframe, so this event is the only way
        // the page learns it happened. Report the same confirmed booking to
        // Google Ads and Meta rather than treating a button click as success.
        const onBooked = (event: {
          detail: { data: BookingConversion };
        }) => {
          reportBookingConversion(event.detail.data);
          reportMetaBooking(event.detail.data);
        };
